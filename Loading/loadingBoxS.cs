...
public class loadingBoxS : MonoBehaviour {
	...
	public void UpdateLabel()
	{
		...
		if (labelCount>1)
		{
			...
			if (hasUpLabel)
			{
				...
				upLabelRenderer = Object1.transform.GetChild(0).GetComponent<Renderer>();
				if (hasHeavyLabel)
				{
					Object2 = Instantiate(HeavyLabelObj, transform.position, Quaternion.identity) as GameObject;
					heavyLabelRenderer = Object2.transform.GetChild(0).GetComponent<Renderer>();
				}
				else
				{
			...
			else
			{
				...
				heavyLabelRenderer = Object1.transform.GetChild(0).GetComponent<Renderer>();
				Object2 = Instantiate(FragileLabelObj, transform.position, Quaternion.identity) as GameObject;
				//fragileLabelRenderer = Object2.transform.GetChild(0).renderer;
			}
		...
		else
		{
			if (hasUpLabel)
			{
				...
				upLabelRenderer = UpObject.transform.GetChild(0).GetComponent<Renderer>();
			...
			if (hasHeavyLabel)
			{
				...
				heavyLabelRenderer = HeavyObject.transform.GetChild(0).GetComponent<Renderer>();
	...
	{
		Collider2D[] colliders = Physics2D.OverlapAreaAll(GetComponent<Collider2D>().bounds.min, GetComponent<Collider2D>().bounds.max);
	...
	{
		if (hasFragileLabel && GetComponent<Renderer>().material.mainTexture != brokenBoxTexture && coll.relativeVelocity.magnitude > 2.0f)
		{
			saverS.failCount++;
			...
			GetComponent<Renderer>().material.mainTexture = brokenBoxTexture;
			GetComponent<AudioSource>().PlayOneShot(sounds[Random.Range(3, 6)]);
			visualM.visualNo = 4;
		...
		else
		{
			...
			{
				GetComponent<AudioSource>().PlayOneShot(sounds[Random.Range(0, 3)], coll.relativeVelocity.magnitude);
			}
		}
