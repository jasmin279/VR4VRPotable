...
//		if(!firstDragOccured && myDragable.draging) firstDragOccured = true;
	...
	void CheckWeight()
	{
		if(weightImportant)
		{
			if(isHeavy)
			{
				heavyLabelObj.GetComponent<Renderer>().material.color = Color.white;
				...
				foreach (RaycastHit2D box in boxesBelow) 
				{
					...
					if(item && item!=this)
					{
						...
						{
							heavyLabelObj.GetComponent<Renderer>().material.color = colors[0];
							wrongLabel = true;
							break;
						}
					}
			...
			{
				lightLabelObj.GetComponent<Renderer>().material.color = Color.white;
				RaycastHit2D[] boxesBelow = Physics2D.BoxCastAll( (Vector2)transform.position, itemSize * 0.9f, transform.eulerAngles.z, Vector2.up, 2.0f);
				foreach (RaycastHit2D box in boxesBelow) 
				{
					loadingItemS item = box.transform.GetComponent<loadingItemS>();
					...
					if(item && item!=this)
					{
						...
						{
							lightLabelObj.GetComponent<Renderer>().material.color = colors[0];
							wrongLabel = true;
							break;
						}
					}
	...
	void CheckDirection()
	{
		...
		{
			upLabelObj.GetComponent<Renderer>().material.color = Color.white;
			if (Vector3.Dot(transform.up, Vector3.up) < 0.95f ) 
			{
				upLabelObj.GetComponent<Renderer>().material.color = colors[0];
				wrongLabel = true;
			}
		}
	}
