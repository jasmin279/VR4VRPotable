...
//		}
		...
		if(virtualActive)
		{
			if(RightHandActivated && LeftHandActivated && !transform.GetComponent<Rigidbody>().isKinematic && !droping)
			{
				...
				RHModel.GetComponent<Collider>().isTrigger = true;
				LHModel.GetComponent<Collider>().isTrigger = true;
			...
			if(!(RightHandActivated && LeftHandActivated) && transform.GetComponent<Rigidbody>().isKinematic)
			{
			...
			if(transform.GetComponent<Rigidbody>().isKinematic)
			{
				...
				*/
				...
				if( (LastLtoR.magnitude > InitialLtoRDistance*1.1f ) && droping==false)
				{
					...
					RHModel.GetComponent<Collider>().isTrigger = false;
					LHModel.GetComponent<Collider>().isTrigger = false;	
					...
					{
						transform.GetComponent<Rigidbody>().velocity = LastSpeed.magnitude < 2.0f ? LastSpeed : LastSpeed.normalized*2.0f;
						transform.GetComponent<Rigidbody>().angularVelocity = LastAngularSpeed;
					}
				}
			}
			...
			{
				RHModel.transform.GetComponent<Collider>().isTrigger = false;
				LHModel.transform.GetComponent<Collider>().isTrigger = false;
				droping = false;			
			}
